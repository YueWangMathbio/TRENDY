"""
the training file for GENIE3-rev method
use genie3 and then use TE(k=2) model, trained on all time points
"""
...
# Load input and target data, stack datasets across time points
def load_data():
    x_data = []
    x_k_data = []
    y_data = []
    ...
    for time_point in range(10):
        for i in data_ranges:
            ...
            x_file = os.path.join(data_folder, f'dataset_{ds_num}_total_genie.npy')
            ...
            x = np.load(x_file)[:, time_point, :, :]  # Extract time_point from each file
            x_k = np.load(xp_file)[:, time_point + 1, :, :]
            y = np.load(y_file)
    ...
    x_k_data = np.vstack(x_k_data)
    y_data = np.vstack(y_data)
    
    return torch.tensor(x_data, dtype=torch.float32, device=device), torch.tensor(x_k_data, dtype=torch.float32, device=device), torch.tensor(y_data, dtype=torch.float32, device=device)

x_train, x_k_train, y_train = load_data()

# Load validation data across all time points
x_val = []
x_k_val = []
y_val = []
for time_point in range(10):
    x_val.append(np.load(os.path.join(data_folder, 'dataset_val_total_genie.npy'))[:, time_point, :, :])
    x_k_val.append(np.load(os.path.join(data_folder, 'dataset_val_total_cov.npy'))[:, time_point + 1, :, :])
    y_val.append(np.load(os.path.join(data_folder, 'dataset_val_total_A.npy')))
x_val = np.vstack(x_val)
x_k_val = np.vstack(x_k_val)
y_val = np.vstack(y_val)
x_val = torch.tensor(x_val, dtype=torch.float32, device=device)
x_k_val = torch.tensor(x_k_val, dtype=torch.float32, device=device)
y_val = torch.tensor(y_val, dtype=torch.float32, device=device)  

# Create data loader for training
batch_size = 64
train_dataset = torch.utils.data.TensorDataset(x_train, x_k_train, y_train)
...
for epoch in range(num_epochs):
    ...
    with tqdm(train_loader, desc=f"Epoch {epoch+1}/{num_epochs}", unit="batch") as progress_bar:
        for inputs0, inputs1, targets in progress_bar:
            optimizer.zero_grad()
    ...
    with torch.no_grad():
        val_auroc_sum = 0.0
        val_auprc_sum = 0.0
        y_pred_all = model(x_val, x_k_val).cpu().numpy()
        for i in range(x_val.size(0)):
            y_true = y_val[i].cpu().numpy()
            y_pred = y_pred_all[i]
...
torch.save(model.state_dict(), "genie_rev.pth")
