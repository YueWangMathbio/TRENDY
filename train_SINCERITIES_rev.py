"""
the training file for SINCERITIES-rev method
use sincerities and then use TE(k=1) model, trained on all time points
"""
...
# Load input and target data, stack all datasets into tensors
def load_data():
    x_data = []
    y_data = []
    data_ranges = list(range(0, 100))
    for i in data_ranges:
        ds_num = f'{data_ranges[i]:02d}'  # Ensure consistent file naming format
        x_file = os.path.join(data_folder, f'dataset_{ds_num}_total_sinc.npy')
        y_file = os.path.join(data_folder, f'dataset_{ds_num}_total_A.npy')
        x = np.load(x_file)
        y = np.load(y_file)  # Load target matrices
...
x_val = np.load(os.path.join(data_folder, 'dataset_val_total_sinc.npy'))
...
# Custom loss function that excludes diagonal elements
...
# Training configuration and early stopping setup
num_epochs = 100
model.train()
...
# Calculate initial validation AUROC and AUPRC on raw data
initial_val_auroc_sum = 0.0
initial_val_auprc_sum = 0.0
for i in range(x_val.size(0)):  # Iterate over validation samples
...
torch.save(model.state_dict(), "weights/sinc_rev.pth")
